<?php
// FILE: ph_messages.php (Messages View)
// VERSION 4: Logic moved to top, Delete function added, Bugs fixed.

// --- (NEW) ALL PHP LOGIC MOVED TO TOP ---
// This ensures redirects (header()) work before any HTML is sent.
if (session_status() === PHP_SESSION_NONE) {
    session_start();
}
include_once 'db.php'; // DB connection is needed for all logic

// --- Authorization Check (from ph_header.php, needed early) ---
if (!isset($_SESSION['role']) || ($_SESSION['role'] !== 'Programhead')) {
    header("Location: index.php");
    exit();
}
$current_user_id = $_SESSION['sid'];
$current_view = 'messages'; // Set the active link
$start_conversation_error = '';
$send_error = '';
$db_error = ''; // General DB error
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_conversation') {
    ...
    if ($conversation_id_to_delete) {
        try {
            // We need to know if we are user_1 or user_2 to hide it correctly
        ...
        if ($recipient_id == $current_user_id) {
            ...
            try {
                ...
                if ($existing_conversation) {
                    header("Location: ph_messages.php?conversation_id=" . $existing_conversation['conversation_id']);
                    exit();
                } else {
                    ...
                    header("Location: ph_messages.php?conversation_id=" . $new_conversation_id);
                    exit();
                }
            } catch (PDOException $e) {
...
$selected_conversation_id = $_GET['conversation_id'] ?? null; // Get convo ID for sending
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_message']) && $selected_conversation_id) {
    ...
    if (!empty($body) && $receiver_id) {
        try {
            ...
            // ===================================================================
            // BUG FIX 1: Added 'subject' column to satisfy NOT NULL constraint
            // ===================================================================
            ...
            // This redirect is what makes it "real-time" (it re-fetches the data)
            header("Location: ph_messages.php?conversation_id=" . $selected_conversation_id);
            exit(); 
        } catch (PDOException $e) {
            $conn->rollBack();
...
// --- (NEW) All logic is done, now we include the header to START sending HTML ---
include_once 'ph_header.php'; 
?>
<style>
    /*
    * INTERNAL STYLES FOR: ph_messages.php
    */
    ...
    .messaging-layout {
        display: flex;
        width: 100%;
        flex-grow: 1; /* Make it fill the page-view */
        height: 100%; /* Ensure it takes full height */
        overflow: hidden;
        background: var(--white);
        position: relative; /* For mobile responsive toggle */
    }
    ...
    /* Stop link navigation when clicking button */
    ...
    .message-header {
        ...
        flex-shrink: 0;
        z-index: 10;
        min-height: 85px; /* Match sidebar header height */
    }
    ...
    #new-conv-form {
        ...
        gap: 8px;
        justify-content: flex-end;
        text-align: right !important; /* Override inline style */
    }
    ...
    @media (max-width: 768px) {
        ...
        /* Your PHP class logic will now work */
...
<?php
// --- (NEW) Page-Specific Data Fetching (runs *after* header) ---
...
// --- Fetch data specific to the 'messages' view ---
if ($selected_conversation_id) {
    
    // ===================================================================
    // BUG FIX 2: Fetch conversation details directly from DB
    // ===================================================================
    $stmt_conv_details = $conn->prepare("
        SELECT 
            c.user_1_id, c.user_2_id,
            u1.role as user_1_role,
            u2.role as user_2_role,
            CONCAT(up1.first_name, ' ', up1.last_name) as user_1_name,
            CONCAT(up2.first_name, ' ', up2.last_name) as user_2_name
    ...
    $conv = $stmt_conv_details->fetch(PDO::FETCH_ASSOC);

    if ($conv) {
        $is_part_of_conversation = true;
        
        // Now, correctly determine the other user
        if ($conv['user_1_id'] == $current_user_id) {
            $current_conversation_receiver_id = (int)$conv['user_2_id'];
            $other_user_name = trim($conv['user_2_name']) ?: 'User';
            $other_user_role = $conv['user_2_role'] ?? 'User';
        } else {
            $current_conversation_receiver_id = (int)$conv['user_1_id'];
            $other_user_name = trim($conv['user_1_name']) ?: 'User';
            $other_user_role = $conv['user_1_role'] ?? 'User';
        }

    } else {
        // User is not part of this conversation or it doesn't exist
        $is_part_of_conversation = false;
    }
    // ===================================================================
    // END OF BUG FIX 2
    ...
    if ($is_part_of_conversation) {
        ...
        $selected_conversation_id = null; // Invalidate
        $other_user_name = "Invalid Conversation";
        // Only set this error if another one isn't already set
        if (empty($db_error)) {
...
// --- Fetch last message for the sidebar ---
// $conversations is already loaded from ph_header.php
...
<div class="messaging-layout <?php echo $selected_conversation_id ? 'show-chat' : 'show-list'; ?>">
            ...
            <?php else: ?>
                <?php foreach ($conversations as $conv): ?>
                    <?php
                        ...
                        if (empty($initials)) {
                            $initials = 'U'; // Fallback for User
                        }
                    ?>
                    <a href="ph_messages.php?conversation_id=<?= $conv['conversation_id'] ?>" 
                       class="conversation-item <?= $is_active ? 'active' : '' ?>">
                        
                        <form method="POST" action="ph_messages.php" class="delete-conversation-btn-wrapper" onsubmit="return confirm('Are you sure you want to hide this conversation?');">
    ...
    <div class="message-view">
        <header class="message-header">
            <a href="ph_messages.php" class="back-button">
        ...
        <footer class="message-input-area">
            ...
            <form action="ph_messages.php?conversation_id=<?= $selected_conversation_id; ?>" method="POST">
...
<div id="new-conversation-modal" class="modal <?php if(!empty($start_conversation_error)) echo 'show'; ?>">
    <div class="modal-content">
        ...
        <form action="ph_messages.php" method="POST" id="new-conv-form">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (searchInput) {
        searchInput.addEventListener('input', () => {
            ...
            // You must have 'ph_ajax_search_users.php' in the same directory
            fetch(`ph_ajax_search_users.php?query=${encodeURIComponent(query)}`)
